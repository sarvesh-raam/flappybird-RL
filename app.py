...
def game_loop():
    ...
    while True:
        ...
        # Target ~45hz for high fluidity on localhost
        elapsed = time.time() - loop_start
        sleep_time = max(0.002, 0.022 - elapsed)
...
def gen_frames():
    ...
    last_frame = None
    while True:
        # Deliver frames as fast as they are generated by the game loop
        if game.latest_frame and game.latest_frame != last_frame:
            last_frame = game.latest_frame
            yield (b'--frame\r\n'
                   b'Content-Type: image/jpeg\r\n\r\n' + game.latest_frame + b'\r\n')
        else:
            time.sleep(0.01) # Ultra-short wait to avoid CPU pegging while waiting for next frame
